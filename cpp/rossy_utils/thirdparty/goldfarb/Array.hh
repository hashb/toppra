...
inline GVect<T> sqrt(const GVect<T>& v) {
  GVect<T> tmp(v.size());
  for (unsigned int i = 0; i < v.size(); i++) tmp[i] = sqrt(v[i]);

  return tmp;
}

template <typename T>
GVect<T>::GVect() : n(0), v(0) {}

template <typename T>
GVect<T>::GVect(const unsigned int n) : v(new T[n]) {
  this->n = n;
}
...
template <typename T>
GVect<T>::GVect(const GVect<T>& rhs) : v(new T[rhs.n]) {
  this->n = rhs.n;
  for (unsigned int i = 0; i < n; i++) v[i] = rhs[i];
}
...
template <typename T>
inline const T& GVect<T>::operator[](
...
inline void GVect<T>::set(const T* a, unsigned int n) {
  resize(n);
  for (unsigned int i = 0; i < n; i++) v[i] = a[i];
}
...
inline GVect<T> operator+(const GVect<T>& lhs, const GVect<T>& rhs) {
  ...
  for (unsigned int i = 0; i < lhs.size(); i++) tmp[i] = lhs[i] + rhs[i];
...
inline GVect<T> operator+(const GVect<T>& lhs, const T& a) {
  GVect<T> tmp(lhs.size());
  for (unsigned int i = 0; i < lhs.size(); i++) tmp[i] = lhs[i] + a;
...
inline GVect<T> operator+(const T& a, const GVect<T>& rhs) {
  ...
  for (unsigned int i = 0; i < rhs.size(); i++) tmp[i] = a + rhs[i];
...
inline GVect<T> operator^(const GVect<T>& lhs, const GVect<T>& rhs) {
  if (lhs.size() != rhs.size())
    throw std::logic_error("Operator^: vectors have different sizes");
  GVect<T> tmp(lhs.size());
  for (unsigned int i = 0; i < lhs.size(); i++) tmp[i] = pow(lhs[i], rhs[i]);
...
inline GVect<T> operator^(const GVect<T>& lhs, const T& a) {
  GVect<T> tmp(lhs.size());
  for (unsigned int i = 0; i < lhs.size(); i++) tmp[i] = pow(lhs[i], a);
...
inline GVect<T> operator^(const T& a, const GVect<T>& rhs) {
  GVect<T> tmp(rhs.size());
  for (unsigned int i = 0; i < rhs.size(); i++) tmp[i] = pow(a, rhs[i]);
...
inline GVect<T>& GVect<T>::operator^=(const GVect<T>& rhs) {
  if (this->size() != rhs.size())
    throw std::logic_error("Operator^=: vectors have different sizes");
  for (unsigned int i = 0; i < n; i++) v[i] = pow(v[i], rhs[i]);
...
template <typename T>
inline GVect<T>& GVect<T>::operator^=(const T& a) {
  for (unsigned int i = 0; i < n; i++) v[i] = pow(v[i], a);
...
std::istream& operator>>(std::istream& is, GVect<T>& v) {
  ...
  is >> elements;
  v.resize(elements);
  for (unsigned int i = 0; i < elements; i++) is >> v[i] >> comma;
...
inline T sum(const GVect<T>& v) {
  T tmp = (T)0;
  for (unsigned int i = 0; i < v.size(); i++) tmp += v[i];
...
inline T max(const GVect<T>& v) {
  T value = v[0];
  for (unsigned int i = 1; i < v.size(); i++) value = std::max(v[i], value);
...
inline T min(const GVect<T>& v) {
  T value = v[0];
  for (unsigned int i = 1; i < v.size(); i++) value = std::min(v[i], value);
...
inline GVect<T> exp(const GVect<T>& v) {
  GVect<T> tmp(v.size());
  for (unsigned int i = 0; i < v.size(); i++) tmp[i] = exp(v[i]);
...
inline GVect<T> log(const GVect<T>& v) {
  GVect<T> tmp(v.size());
  for (unsigned int i = 0; i < v.size(); i++) tmp[i] = log(v[i]);
...
inline GVect<T> pow(const GVect<T>& v, double a) {
  GVect<T> tmp(v.size());
  for (unsigned int i = 0; i < v.size(); i++) tmp[i] = pow(v[i], a);
...
inline GVect<T> abs(const GVect<T>& v) {
  GVect<T> tmp(v.size());
  for (unsigned int i = 0; i < v.size(); i++) tmp[i] = (T)fabs(v[i]);
...
inline GVect<double> rank(const GVect<T>& v) {
  ...
  for (unsigned int i = 0; i < tmp.size(); i++) {
    ...
    if (equal > 0) {
      for (unsigned int j = 1; j <= equal; j++) tmp_rank[i] += smaller + 1 + j;
...
template <typename T>
GMatr<T>::GMatr(const GMatr<T>& rhs) : v(new T*[rhs.n]) {
  unsigned int i, j;
  n = rhs.n;
  m = rhs.m;
  v[0] = new T[m * n];
...
inline GVect<T> GMatr<T>::extractDiag() const {
  ...
  for (i = 0; i < d; i++) tmp[i] = v[i][i];

  return tmp;
}

template <typename T>
inline GMatr<T> GMatr<T>::extractRows(
    const std::set<unsigned int>& indexes) const {
...
template <typename T>
inline GMatr<T> GMatr<T>::extractColumns(
    const std::set<unsigned int>& indexes) const {
...
template <typename T>
inline GMatr<T> GMatr<T>::extract(
...
inline void GMatr<T>::setRow(unsigned int i, const GVect<T>& a) {
  ...
  for (unsigned int j = 0; j < ncols(); j++) v[i][j] = a[j];
}
...
inline void GMatr<T>::setRow(unsigned int i, const GMatr<T>& a) {
  ...
  for (unsigned int j = 0; j < ncols(); j++) v[i][j] = a[0][j];
}
...
inline void GMatr<T>::setColumn(unsigned int j, const GVect<T>& a) {
  ...
  if (this->n != a.size())
    throw std::logic_error(
        "Error setting matrix column: ranges are not compatible");
  for (unsigned int i = 0; i < nrows(); i++) v[i][j] = a[i];
}
...
inline void GMatr<T>::setColumn(unsigned int j, const GMatr<T>& a) {
  ...
  for (unsigned int i = 0; i < nrows(); i++) v[i][j] = a[i][0];
}
...
inline void GMatr<T>::set(const T* a, unsigned int n, unsigned int m) {
  if (this->n != n || this->m != m) resize(n, m);
  unsigned int k = 0;
  for (unsigned int i = 0; i < n; i++)
    for (unsigned int j = 0; j < m; j++) v[i][j] = a[k++];
}
...
GMatr<T> operator+(const GMatr<T>& lhs, const T& a) {
  GMatr<T> tmp(lhs.nrows(), lhs.ncols());
  for (unsigned int i = 0; i < lhs.nrows(); i++)
    for (unsigned int j = 0; j < lhs.ncols(); j++) tmp[i][j] = lhs[i][j] + a;
...
GMatr<T> operator+(const T& a, const GMatr<T>& rhs) {
  ...
  for (unsigned int i = 0; i < rhs.nrows(); i++)
    for (unsigned int j = 0; j < rhs.ncols(); j++) tmp[i][j] = a + rhs[i][j];
...
inline GMatr<T>& GMatr<T>::operator+=(const GMatr<T>& rhs) {
  if (m != rhs.ncols() || n != rhs.nrows())
    throw std::logic_error("Operator+=: matrices have different sizes");
  for (unsigned int i = 0; i < n; i++)
    for (unsigned int j = 0; j < m; j++) v[i][j] += rhs[i][j];
...
inline GMatr<T>& GMatr<T>::operator^=(const GMatr<T>& rhs) {
  if (m != rhs.ncols() || n != rhs.nrows())
    throw std::logic_error("Operator^=: matrices have different sizes");
  for (unsigned int i = 0; i < n; i++)
    for (unsigned int j = 0; j < m; j++) v[i][j] = pow(v[i][j], rhs[i][j]);
...
inline GMatr<T>& GMatr<T>::operator^=(const T& a) {
  for (unsigned int i = 0; i < n; i++)
    for (unsigned int j = 0; j < m; j++) v[i][j] = pow(v[i][j], a);
...
inline GMatr<T>::operator GVect<T>() {
  if (n > 1 && m > 1)
    throw std::logic_error(
        "Error matrix cast to vector: trying to cast a multi-dimensional "
        "matrix");
  if (n == 1)
    return extractRow(0);
  else
...
std::istream& operator>>(std::istream& is, GMatr<T>& m) {
  ...
  for (unsigned int i = 0; i < rows; i++)
    for (unsigned int j = 0; j < cols; j++) is >> m[i][j] >> comma;
...
void svd(const GMatr<T>& A, GMatr<T>& U, GVect<T>& W, GMatr<T>& V) {
  ...
  for (i = n - 1; i >= 0; i--) {
    if (i < n - 1) {
      ...
      for (j = l; j < n; j++) V[i][j] = V[j][i] = (T)0.0;
    }
    V[i][i] = (T)1.0;
    g = rv1[i];
    l = i;
...
int lu(const GMatr<T>& A, GMatr<T>& LU, GVect<unsigned int>& index) {
  ...
  for (i = 0; i < n; i++) {
    index[i] = i;
    val = (T)0.0;
    for (j = 0; j < n; j++) val = std::max(val, (T)fabs(LU[i][j]));
    if (val == (T)0.0)
  ...
  for (k = 0; k < n - 1; k++) {
    ...
    if (p > k) {
      ...
      std::swap(d[k], d[p]);
      for (j = 0; j < n; j++) std::swap(LU[k][j], LU[p][j]);
    }

    for (i = k + 1; i < n; i++) {
      LU[i][k] /= LU[k][k];
      for (j = k + 1; j < n; j++) LU[i][j] -= LU[i][k] * LU[k][j];
    }
  }
  if (LU[n - 1][n - 1] == (T)0.0)
    std::logic_error("Error in LU decomposition: matrix was singular");
...
GVect<T> lu_solve(const GMatr<T>& LU, const GVect<T>& b,
  ...
  if (b.size() != n)
    throw std::logic_error(
        "Error in LU solve: b vector must be of the same dimensions of LU "
        "matrix");
  GVect<T> x((T)0.0, n);
  int i, j, p;
  T sum;
...
void cholesky(const GMatr<T> A, GMatr<T>& LL) {
  ...
  for (i = 0; i < n; i++) {
    for (j = i; j < n; j++) {
      sum = LL[i][j];
      for (k = i - 1; k >= 0; k--) sum -= LL[i][k] * LL[j][k];
      if (i == j) {
        if (sum <= 0.0)
          throw std::logic_error(
              "Error in Cholesky decomposition: matrix is not postive "
              "definite");
        LL[i][i] = std::sqrt(sum);
      } else
        LL[j][i] = sum / LL[i][i];
    }
    for (k = i + 1; k < n; k++) LL[i][k] = LL[k][i];
  }
}
...
GVect<T> cholesky_solve(const GMatr<T>& LL, const GVect<T>& b) {
  ...
  if (b.size() != n)
    throw std::logic_error(
        "Error in Cholesky decomposition: b vector must be of the same "
...
void forward_elimination(const GMatr<T>& L, GVect<T>& y, const GVect<T> b) {
  if (L.ncols() != L.nrows())
    throw std::logic_error(
        "Error in Forward elimination: matrix must be squared (lower "
        "triangular)");
  if (b.size() != L.nrows())
    throw std::logic_error(
        "Error in Forward elimination: b vector must be of the same dimensions "
        "of L matrix");
  int i, j, n = b.size();
  y.resize(n);
...
void backward_elimination(const GMatr<T>& U, GVect<T>& x, const GVect<T>& y) {
  if (U.ncols() != U.nrows())
    throw std::logic_error(
        "Error in Backward elimination: matrix must be squared (upper "
        "triangular)");
  if (y.size() != U.nrows())
    throw std::logic_error(
        "Error in Backward elimination: b vector must be of the same "
        "dimensions of U matrix");
  int i, j, n = y.size();
  x.resize(n);
...
GVect<T> sum(const GMatr<T>& m) {
  ...
  for (unsigned int j = 0; j < m.ncols(); j++)
    for (unsigned int i = 0; i < m.nrows(); i++) tmp[j] += m[i][j];
  return tmp;
}
...
GVect<T> r_sum(const GMatr<T>& m) {
  ...
  for (unsigned int i = 0; i < m.nrows(); i++)
    for (unsigned int j = 0; j < m.ncols(); j++) tmp[i] += m[i][j];
  return tmp;
}
...
T all_sum(const GMatr<T>& m) {
  T tmp = (T)0;
  for (unsigned int i = 0; i < m.nrows(); i++)
    for (unsigned int j = 0; j < m.ncols(); j++) tmp += m[i][j];
  return tmp;
}
...
GVect<T> mean(const GMatr<T>& m) {
  ...
  for (unsigned int j = 0; j < m.ncols(); j++) {
    for (unsigned int i = 0; i < m.nrows(); i++) res[j] += m[i][j];
    res[j] /= m.nrows();
  }
...
GVect<T> r_mean(const GMatr<T>& m) {
  ...
  for (unsigned int i = 0; i < m.nrows(); i++) {
    for (unsigned int j = 0; j < m.ncols(); j++) res[i] += m[i][j];
    res[i] /= m.nrows();
  }
...
GMatr<T> exp(const GMatr<T>& m) {
  ...
  for (unsigned int i = 0; i < m.nrows(); i++)
    for (unsigned int j = 0; j < m.ncols(); j++) tmp[i][j] = exp(m[i][j]);
...
GMatr<T> sqrt(const GMatr<T>& m) {
  ...
  for (unsigned int i = 0; i < m.nrows(); i++)
    for (unsigned int j = 0; j < m.ncols(); j++) tmp[i][j] = sqrt(m[i][j]);
...
GMatr<T> kron(const GVect<T>& b, const GVect<T>& a) {
  ...
  for (unsigned int i = 0; i < b.size(); i++)
    for (unsigned int j = 0; j < a.size(); j++) tmp[i][j] = a[j] * b[i];
...
GMatr<T> t(const GMatr<T>& a) {
  ...
  for (unsigned int i = 0; i < a.nrows(); i++)
    for (unsigned int j = 0; j < a.ncols(); j++) tmp[j][i] = a[i][j];
...
GMatr<T> dot_prod(const GMatr<T>& a, const GMatr<T>& b) {
  if (a.ncols() != b.nrows())
    throw std::logic_error(
        "Error matrix dot product: dimensions of the matrices are not "
        "compatible");
...
GMatr<T> dot_prod(const GMatr<T>& a, const GVect<T>& b) {
  if (a.ncols() != b.size())
    throw std::logic_error(
        "Error matrix dot product: dimensions of the matrix and the vector are "
        "not compatible");
  GMatr<T> tmp(a.nrows(), 1);
  for (unsigned int i = 0; i < tmp.nrows(); i++) {
    tmp[i][0] = (T)0;
    for (unsigned int k = 0; k < a.ncols(); k++) tmp[i][0] += a[i][k] * b[k];
  }
...
GMatr<T> dot_prod(const GVect<T>& a, const GMatr<T>& b) {
  if (a.size() != b.ncols())
    throw std::logic_error(
        "Error matrix dot product: dimensions of the vector and matrix are not "
        "compatible");
  GMatr<T> tmp(1, b.ncols());
  for (unsigned int j = 0; j < tmp.ncols(); j++) {
    tmp[0][j] = (T)0;
    for (unsigned int k = 0; k < a.size(); k++) tmp[0][j] += a[k] * b[k][j];
  }
