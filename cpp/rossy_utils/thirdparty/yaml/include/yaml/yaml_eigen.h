...
struct convert<
  ...
  static Node encode(const Eigen::Matrix<_Scalar, _Rows, _Cols, _Options,
  ...
  static bool decode(const Node& node,
    ...
    if (_Rows == 1 || _Cols == 1) {
      (_Rows == 1 ? matrix.resize(_Rows, node.size())
                  : matrix.resize(node.size(), _Cols));
      for (int id = 0; id < node.size(); id++)
        (node[0].size() == 0 ? matrix(id) = node[(int)id].as<_Scalar>()
                             : matrix(id) = node[(int)id][0].as<_Scalar>());
      return true;
    }
