...
void SingleDocParser::HandleDocument(EventHandler& eventHandler) {
  ...
  // eat doc start
  if (m_scanner.peek().type == Token::DOC_START) m_scanner.pop();
...
void SingleDocParser::HandleBlockSequence(EventHandler& eventHandler) {
  ...
  while (1) {
    ...
    m_scanner.pop();
    if (token.type == Token::BLOCK_SEQ_END) break;
...
void SingleDocParser::ParseTag(std::string& tag) {
  Token& token = m_scanner.peek();
  if (!tag.empty()) throw ParserException(token.mark, ErrorMsg::MULTIPLE_TAGS);
...
void SingleDocParser::ParseAnchor(anchor_t& anchor) {
  Token& token = m_scanner.peek();
  if (anchor) throw ParserException(token.mark, ErrorMsg::MULTIPLE_ANCHORS);
...
anchor_t SingleDocParser::RegisterAnchor(const std::string& name) {
  if (name.empty()) return NullAnchor;
