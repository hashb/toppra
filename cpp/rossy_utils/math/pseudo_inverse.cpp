...
void pseudoInverse(Eigen::MatrixXd const& matrix, double sigmaThreshold,
  ...
  for (int ii(0); ii < nrows; ++ii) {
    ...
    if (sii > 2 * sigmaThreshold)
      ...
      invS.coeffRef(ii, ii) =
          sii / (sii * sii + sigmaThreshold * sigmaThreshold);
  }
  ...
  // Eigen::MatrixXd J = svd.matrixU() * ss.asDiagonal() *
...
void weightedInverse(const Eigen::MatrixXd& J, const Eigen::MatrixXd& Winv,
                     Eigen::MatrixXd& Jinv) {
