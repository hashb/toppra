...
class GRIP_DATA {
 public:
  ...
  void setBoxObject(double mass, double box_x, double box_y, double box_z,
                    double pcom) {
    m = mass;
 ...
 protected:
  void _computeFrictionConeMatrix(int n, double mu, double r_pad, double x,
                                  double y, const Eigen::MatrixXd& AdT_ti) {
    //---- compute U : inequality matrix

    Ut = Eigen::MatrixXd::Zero(12, 6);
    Ut << 0., 0., 0., mu, mu, -1, 0., 0., 0., mu, -mu, -1, 0., 0., 0., -mu, mu,
        -1, 0., 0., 0., -mu, -mu, -1, mu, mu, -1, -y, -x, -(x + y) * mu, mu,
        -mu, -1, -y, x, -(x + y) * mu, -mu, mu, -1, y, -x, -(x + y) * mu, -mu,
        -mu, -1, y, x, -(x + y) * mu, mu, mu, -1, y, x, -(x + y) * mu, mu, -mu,
    ...
    for (int i(0); i < n; ++i) U.block(i * nr, i * nc, nr, nc) = Ui;
  }
