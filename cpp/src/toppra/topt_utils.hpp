...
Eigen::MatrixXd hZeroStack(int Nl, const Eigen::MatrixXd& A) {
  // [zero(Nl), A]
  int r = A.rows();
  return rossy_utils::hStack(Eigen::MatrixXd::Zero(r, Nl), A);
}
Eigen::MatrixXd hZeroStack(const Eigen::MatrixXd& A, int Nr) {
  // [A, zero(Nr)]
  int r = A.rows();
  return rossy_utils::hStack(A, Eigen::MatrixXd::Zero(r, Nr));
}
Eigen::MatrixXd hZeroStack(int Nl, const Eigen::MatrixXd& A, int Nr) {
  // [zero(Nl), A, zero(Nr)]
  int r = A.rows();
  Eigen::MatrixXd temp = rossy_utils::hStack(Eigen::MatrixXd::Zero(r, Nl), A);
  return rossy_utils::hStack(temp, Eigen::MatrixXd::Zero(r, Nr));
}
