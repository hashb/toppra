...
void Scanner::SimpleKey::Invalidate() {
  if (pIndent) pIndent->status = IndentMarker::INVALID;
  if (pMapStart) pMapStart->status = Token::INVALID;
  if (pKey) pKey->status = Token::INVALID;
}

// CanInsertPotentialSimpleKey
bool Scanner::CanInsertPotentialSimpleKey() const {
  if (!m_simpleKeyAllowed) return false;
...
bool Scanner::ExistsActiveSimpleKey() const {
  if (m_simpleKeys.empty()) return false;
...
void Scanner::InsertPotentialSimpleKey() {
  if (!CanInsertPotentialSimpleKey()) return;
...
void Scanner::InvalidateSimpleKey() {
  if (m_simpleKeys.empty()) return;

  // grab top key
  SimpleKey& key = m_simpleKeys.top();
  if (key.flowLevel != GetFlowLevel()) return;
...
bool Scanner::VerifySimpleKey() {
  ...
  if (key.flowLevel != GetFlowLevel()) return false;
...
void Scanner::PopAllSimpleKeys() {
  while (!m_simpleKeys.empty()) m_simpleKeys.pop();
}
}  // namespace YAML
