...
void Scanner::ScanDirective() {
  ...
  while (1) {
    ...
    // now read parameter
    std::string param;
    while (INPUT && !Exp::BlankOrBreak().Matches(INPUT)) param += INPUT.get();
...
void Scanner::ScanFlowEnd() {
  if (InBlockContext()) throw ParserException(INPUT.mark(), ErrorMsg::FLOW_END);
...
void Scanner::ScanAnchorOrAlias() {
  ...
  // now eat the content
  while (INPUT && Exp::Anchor().Matches(INPUT)) name += INPUT.get();
...
void Scanner::ScanBlockScalar() {
  ...
  if (Exp::Comment().Matches(INPUT))
    while (INPUT && !Exp::Break().Matches(INPUT)) INPUT.eat(1);
  ...
  if (GetTopIndent() >= 0) params.indent += GetTopIndent();
