...
void size_print(Eigen::MatrixXd const& vv, std::ostream& os,
  ...
  if (!title.empty()) {
    os << title << "(rows: " << vv.rows() << ", cols: " << vv.cols() << ")"
       << nlornot;
  }
}
...
void pretty_print(Eigen::MatrixXd const& mm, std::ostream& os,
  ...
  if (!title.empty()) {
    os << title << "(rows: " << mm.rows() << ", cols: " << mm.cols() << ")"
       << nlornot;
  }
...
//     double ret;
//     ret =
//         ini + (end - ini) * 0.5 * (1 - cos(curr_time / moving_duration *
//         M_PI));
...
//     return ret;
// }
// void getSinusoidTrajectory(double initTime_, const Eigen::VectorXd&
// midPoint_,
