...
void NaturalQuarticSplines::computeOpt() {
  ...
  for (int i(0); i < n_wpts; ++i) bs[i] = (ys[i + 1] - ys[i]) / hs[i];
  ...
  // build zs
  zs.resize(n_wpts + 1);
  for (int i(0); i < n_wpts + 1; ++i) zs[i] = znC(i);
...
void NaturalQuarticSplines::computeNatural() {
  ...
  for (int i(0); i < n_wpts - 2; ++i) {
    for (int j = i + 2; j < n_wpts - 1; ++j) A(i, j) = 2. * hhs[i] * hhs[j];
  }
  ...
  zs.resize(n_wpts + 1);
  zs[0] = 0.;
  for (int i(0); i < n_wpts - 1; ++i) zs[i + 1] = z(i);
  zs[n_wpts] = 0.;
...
double NaturalQuarticSplines::evaluate(const double& t_in) {
  ...
  if (i < 0)
    ...
    s = (zs[i + 1] * t1 * t1 * t1 * t1) / (24. * hs[i]) -
        (zs[i] * t2 * t2 * t2 * t2) / (24. * hs[i]) + Cs[i] * t1 * t2 +
        Ds[i] * t1 + Es[i] * t2;
    return s;
  } else
    return ys[n_wpts];
...
double NaturalQuarticSplines::evaluateFirstDerivative(const double& t_in) {
  ...
  sdot = (zs[i + 1] * t1 * t1 * t1) / (6. * hs[i]) +
         (zs[i] * t2 * t2 * t2) / (6. * hs[i]) + Cs[i] * (t2 - t1) + Ds[i] -
         Es[i];
...
double NaturalQuarticSplines::evaluateSecondDerivative(const double& t_in) {
  ...
  double t1 = (t_in - ts[i]);
  double t2 = (ts[i + 1] - t_in);
  sddot = zs[i + 1] * t1 * t1 / (2. * hs[i]) - zs[i] * t2 * t2 / (2. * hs[i]) -
          2. * Cs[i];
  return sddot;
}
...
Eigen::Quaterniond NQSpln4Rot::evaluate(const double& t_in) {
  ...
  if (delq_vec.norm() < 1e-6)
    ...
    delq = Eigen::Quaterniond(
        Eigen::AngleAxisd(delq_vec.norm(), delq_vec / delq_vec.norm()));
  return (q_init * delq);
}
