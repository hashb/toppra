...
void HermiteQuinticSplines::push_back(double t, double y, double ydot,
                                      double yddot) {
  computed = false;
  ts.push_back(t);
  ys.push_back(y);
...
void HermiteQuinticSplines::compute() {
  ...
  H << 1., 0., 0., -10., 15., -6., 0., 1., 0., -6., 8., -3., 0., 0., 0.5, -1.5,
      1.5, -0.5, 0., 0., 0., 0.5, -1., 0.5, 0., 0., 0., -4., 7., -3., 0., 0.,
      0., 10., -15., 6.0;
  ...
  for (int i(0); i < n_wpts; ++i) {
    ...
    tmp << ys[i], dys[i] * tk, ddys[i] * tk * tk, ddys[i + 1] * tk * tk,
        dys[i + 1] * tk, ys[i + 1];
    ci = H.transpose() * tmp;
    coeffs.push_back(ci);
  }
...
void HQSpln4Vec::push_back(double t, const Eigen::VectorXd& Y,
