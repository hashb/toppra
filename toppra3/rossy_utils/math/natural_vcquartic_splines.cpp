...
void NaturalVCQuarticSplines::push_back(double t, double y, double ydot) {
  computed = false;
  ts.push_back(t);
  ys.push_back(y);
  dys.push_back(ydot);
...
void NaturalVCQuarticSplines::compute() {
  ...
  // build hs
  hs.resize(n_wpts);
  for (int i(0); i < n_wpts; ++i) hs[i] = ts[i + 1] - ts[i];
  ...
  // build zs
  zs.resize(n_wpts + 1);
  for (int i(0); i < n_wpts + 1; ++i) zs[i] = z(i);

  // build Cs,Ds,Es
  Cs.resize(n_wpts);
  for (int i(0); i < n_wpts; ++i) Cs[i] = 0.5 * zs[i] * hs[i] * hs[i];

  Ds.resize(n_wpts);
  for (int i(0); i < n_wpts; ++i)
    Ds[i] = 4. * (ys[i + 1] - ys[i]) - (dys[i + 1] + 3. * dys[i]) * hs[i] -
            zs[i] * hs[i] * hs[i];

  Es.resize(n_wpts);
  for (int i(0); i < n_wpts; ++i)
    Es[i] = -3 * (ys[i + 1] - ys[i]) + (dys[i + 1] + 2. * dys[i]) * hs[i] +
            0.5 * zs[i] * hs[i] * hs[i];
...
double NaturalVCQuarticSplines::evaluate(const double& t_in) {
  ...
  if (i < 0)
    ...
    s = ys[i] + (dys[i] * hs[i]) * t + Cs[i] * t * t + Ds[i] * t * t * t +
        Es[i] * t * t * t * t;
    return s;
  } else
    return ys[n_wpts];
...
double NaturalVCQuarticSplines::evaluateFirstDerivative(const double& t_in) {
  ...
  if (i < 0)
    ...
    double t = (t_in - ts[i]) / (ts[i + 1] - ts[i]);
    sdot = (dys[i] * hs[i]) + 2. * Cs[i] * t + 3. * Ds[i] * t * t +
           4. * Es[i] * t * t * t;
    return sdot / hs[i];
  } else
    return 0.;
...
double NaturalVCQuarticSplines::evaluateSecondDerivative(const double& t_in) {
  ...
  if (i < 0)
    ...
    double t = (t_in - ts[i]) / (ts[i + 1] - ts[i]);
    sddot = 2. * Cs[i] + 6. * Ds[i] * t + 12. * Es[i] * t * t;
...
void NVCQSpln4Vec::push_back(double t, const Eigen::VectorXd& Y,
...
void NVCQSpln4Vec::compute() {
  computed = true;
  for (int i(0); i < dim; ++i) curves[i].compute();
}

Eigen::VectorXd NVCQSpln4Vec::evaluate(const double& t_in) {
  output = Eigen::VectorXd::Zero(dim);
  for (int i(0); i < dim; ++i) output[i] = curves[i].evaluate(t_in);
  return output;
}
