...
void TOPPHermiteSplines::compute() {
  ...
  int i = n_wpts - 1;
  ti = 6. * (ps[n_wpts] - ps[n_wpts - 1]) /
       (2. * vs[n_wpts - 1] + 4. * vs[n_wpts]);
  ...
  while (i-- > 0) {
    ...
    vi2 = vs[i + 2];
    ti1 = ti;
    ti = (6. * pi2 - 6. * pi0 - (4. * vi1 + 2. * vi2) * ti1) /
         (2. * vi0 + 4. * vi1);
    if (ti < 0) {
      std::cout << "!!!! ----- TOPPHermiteSplines error ----- !!!!"
                << std::endl;
      ti = 1e-8;
    }
    delt_inv_ord.push_back(ti);
...
void TOPPHermiteSplines::check() {
  ...
  std::cout << " ---TOPPHermiteSplines : total period = " << ts[n_wpts]
            << std::endl;
...
double TOPPHermiteSplines::evaluate(const double& t_in) {
  ...
  if (i < 0)
    ...
    p = (2 * s * s * s - 3. * s * s + 1.) * ps[i] +
        (-2. * s * s * s + 3. * s * s) * ps[i + 1] +
