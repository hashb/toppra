...
void TrajectoryManager::setT2SSpline(const std::vector<double>& s_list,
  ...
  for (int i(0); i < nwpts; ++i) {
    ...
    if (i == nwpts - 1) {
      ...
      u = (xmax_list[i + 1] - xmax_list[i]) / 2. / (s_list[i + 1] - s_list[i]);
    }
...
// if NCSpln4Vec
void TrajectoryManager::setT2QSpline(const std::vector<double>& s_list,
                                     const std::vector<double>& xmax_list) {
  use_t2s_ = false;
  use_t2q_ = true;
...
//         Eigen::VectorXd ddq =
//         spline_s2q_.evaluateSecondDerivative(s_list[i]); qddot = dq*sddot +
//         ddq*sdot*sdot;
...
void TrajectoryManager::getCommand(double t, Eigen::VectorXd& q_cmd) {
  assert(use_t2q_ || use_t2s_);
  if (use_t2q_) q_cmd = spline_t2q_.evaluate(t);
  if (use_t2s_) {
 ...
 * 3. Sample new waypoints uniformly from the spline
 *
 * The number of output waypoints is clamped between MIN_NWPTS_ and MAX_NWPTS_
 *
 * @param qwpts0 Input waypoint sequence
...
void TrajectoryManager::redistQwptsPureNormDist(
  ...
  // Sample new waypoints uniformly from the spline
  qwpts.clear();
...
void TrajectoryManager::redistQwptsNormDist(
  ...
  while (!monoincrease) {
    ...
    for (int i(0); i < num_wpts; i++) {
      ...
      if (i > 0 && slist[i] < slist[i - 1]) monoincrease = false;
    }
  }
...
void TrajectoryManager::redistQwpts2nd(std::vector<Eigen::VectorXd>& qwpts) {
