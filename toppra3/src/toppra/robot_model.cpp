...
Eigen::Matrix<double, 6, 1> RobotSystem::getBodyNodeSpatialVelocity(
...
Eigen::VectorXd RobotSystem::getBodyNodeJacobianDotQDot(
    const std::string& name) {
...
Eigen::Matrix<double, 6, 1> RobotSystem::getBodyNodeBodyVelocity(
    const std::string& name) {
...
Eigen::VectorXd RobotSystem::getBodyNodeBodyJacobianDotQDot(
    const std::string& name) {
...
void RobotSystem::_initializeRobotInfo() {
  ...
  for (pinocchio::JointIndex i(0);
    ...
    if (joint_name != "universe" && joint_name != "root_joint") {
      joint_idx_map_[joint_name] = model_.getJointId(
          joint_name);  // i - 2; joint map excluding fixed joint
      joint_idx_map_inv_[model_.getJointId(joint_name)] =
          joint_name;  // joint map excluding fixed joint
    }
  }
